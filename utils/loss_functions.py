...
def vgg123_loss(x_rec, x_true):
    ...
    loss1 = torch.sum((out1_rec - out1_true).pow(2), dim=[1,2,3])
    loss2 = torch.sum((out2_rec - out2_true).pow(2), dim=[1,2,3])
    loss3 = torch.sum((out3_rec - out3_true).pow(2), dim=[1,2,3])
